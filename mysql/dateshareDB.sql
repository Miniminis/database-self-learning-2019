...
CREATE TABLE dateuser (
	u_num int auto_increment, /* 회원번호 : PRIMARY KEY, ASC*/
	u_id VARCHAR(40) NOT NULL, /* 회원아이디  */
	u_pw VARCHAR(16) NOT NULL, /* 회원비밀번호 */
	u_name VARCHAR(50) NOT NULL, /* 회원이름 */
	u_bday DATE NOT NULL, /* 생년월일 */
	u_regdate datetime default current_timestamp NOT NULL, /* 회원가입날짜 : default sysdate*/ 
...
values(null, 'user1', '111', '유저1', '19/11/11', default, 'M');

insert into dateuser (u_num, u_id, u_pw, u_name, u_bday, u_regdate, u_gender) 
values(null, 'user2', '222', '유저2', '19/11/12', default, 'F');

insert into dateuser (u_num, u_id, u_pw, u_name, u_bday, u_regdate, u_gender) 
values(null, 'user3', '333', '유저3', '19/11/13', default, 'M');

insert into dateuser (u_num, u_id, u_pw, u_name, u_bday, u_regdate, u_gender) 
values(null, 'user4', '444', '유저4', '19/11/14', default, 'F');
...
-- user insert() 
insert into dateuser values(null, 'user5', '111', '유저1' , '19/11/14', default, 'F');
...
-- on delete cascade 이용하여 특정 dateuser 삭제 --> movie 삭제, like 삭제 
delete from dateuser where u_num=2;

insert into movie (m_num, u_num, m_title, m_content, m_writedate, m_path, m_hits, m_star) 
values(null, 2, '제목1', '내용1', default, 1, 1, 1); 

insert into movie 
values(null, 2, '제목3', '내용4', default, 'file.jpg', default, 4);

insert into movie 
values(null, 2, '제목4', '내용5', default, default, default, 4);

insert into movie 
values(null, 2, '제목6', '내용6', default, default, default, 1);
...
insert into movie (m_num, u_num, m_title, m_content, m_writedate, m_path, m_hits, m_star) 
values(null, 2, '제목1', '내용1', default, default, 1, 1); 
...
desc m_like;
select * from m_like;
select count(*) from m_like where u_num=5 and m_num=2;
select count(*) from m_like where m_num=2;
insert into m_like values(2, 2);
delete from m_like where u_num=5 and m_num=6;
insert into m_like values(1, 2);
insert into m_like values(3, 2);
insert into m_like values(4, 2);
drop table m_like;
select * from movie m, dateuser d where m.u_num = d.u_num;
...
CREATE TABLE food (
   ...
   f_star NUMBER DEFAULT 10 NOT NULL NOT NULL, /* 별점 */
